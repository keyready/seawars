...
const updateUserStatsAfterGame = async (winnerName, loserName, roomId) => {
    try {
        const winner = await User.findOne({ username: winnerName });
        const loser = await User.findOne({ username: loserName });

        if (winner) {
            winner.updateGameStats(true);
            await winner.save();
        }

        if (loser) {
            loser.updateGameStats(false);
            await loser.save();
        }
    } catch (error) {
...
const createGameLog = async (room, winner) => {
    ...
    await updateUserStatsAfterGame(winner, opponent, room.id);
